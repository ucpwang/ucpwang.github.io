...
## 하네스: Astro 블로그 통합 워크플로우

**목표:** 블로그 포스트 작성·UI/컴포넌트 변경·디자인 토큰 동기화·배포 전 검증을 4인 서브 에이전트 파이프라인(planner → author → reviewer → verifier)으로 일관되게 처리한다. 메인 클로드가 오케스트레이터 역할로 `Agent` 도구를 통해 각 에이전트를 순차/반복 호출한다.

**트리거:** 블로그 글 작성·수정, 컴포넌트/페이지 변경, 디자인 토큰 수정, `verify.py` 전체 통과 검증 같은 작업을 요청받으면 `astro-harness` 스킬을 사용한다. 단순 질문(파일 위치, 명령어 등)은 직접 응답해도 된다.

**변경 이력:**

| 날짜       | 변경 내용                                           | 대상                                                                                                                                         | 사유                                                                                                                                                                               |
| ---------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-17 | 초기 구성                                           | agents/{planner,author,reviewer,verifier}, skills/{astro-harness,blog-post-author,design-token-review,build-verify}                          | 블로그 포스트 작성 중심의 통합 하네스 신규 구축                                                                                                                                    |
| 2026-05-17 | 오케스트레이터를 표준 서브 에이전트 패턴으로 재조정 | skills/astro-harness/SKILL.md, agents/\*.md(통신 프로토콜 섹션), settings.json(verifier 권한 보강)                                           | 클로드코드 표준 환경에 TeamCreate/SendMessage/TaskCreate 도구가 없음 — Agent + TodoWrite로 동등 구현으로 전환                                                                      |
| 2026-05-17 | 블로그 전체 리셋 + verify.py 자가발견형 전환        | src/content/blog/(10개 삭제), scripts/verify.py, .claude/skills/{astro-harness,build-verify}/SKILL.md, .claude/agents/verifier.md, CLAUDE.md | 기존 포스트가 CISO 정체성과 어긋남. 향후 3축(개발×보안/클라우드 보안/리더십 회고) 재정립. 동시에 콘텐츠 변경마다 verify.py·문서 수정해야 하던 비효율 제거 — 동적 발견형으로 재설계 |
...
- `N/N 전체 통과`(verify.py 출력 마지막 줄)가 확인된 후에만 커밋·배포한다
- 실패 항목이 있으면 수정 → 재빌드 → 재검증 반복
- 검증 스크립트(`scripts/verify.py`)는 **자가발견형** — `src/content/blog/*.md`를 글로빙해 포스트·태그를 동적으로 검증한다. 콘텐츠 추가·삭제 시 verify.py를 손댈 필요 없음
- 새 라우트/컴포넌트가 추가될 때만 verify.py에 `check()` 추가. 절대 카운트는 박지 말고 `len(posts)` 같은 동적 값 사용
